                             ...
                             void *data, uint64_t size)
{
    ...
    while (size) {
        ...
        info_report("%s: size %u, hdr %u, entry %u", __func__,
                    efilist->siglist_size, efilist->header_size, efilist->sig_size);
        ...
        if (qemu_uuid_is_equal(&efilist->guid_type, &EfiCertX509Guid)) {
            info_report("%s:   type: x509 cert", __func__);
            ...
            info_report("%s:   type: sha256 hashes", __func__);
            ...
            error_report("%s: unknown type (%s)", __func__, str_uuid);
            g_free(str_uuid);
        }
