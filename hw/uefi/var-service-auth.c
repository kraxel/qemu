...
static bool secure_boot_is_active(uefi_vars_state *uv)
{
    uefi_variable *var;
    uint8_t *value;
    ...
    assert(var != NULL);
    value = var->data;
    return value[0] == SECURE_BOOT_MODE_ENABLE;
}
                                  ...
                                  mm_variable_access *va, void *data)
{
    ...
    if (uefi_vars_is_sb_any(var)) {
        /* secure boot signature databases update */
        if (auth->hdr_length == 24) {
            /* no signature (auth->cert_data is empty), allow in custom mode */
            ...
            if (!siglist) {
                /* list of trusted certificates is empty */
                if (uefi_vars_is_sb_pk(var)) {
                    ...
                    status = uefi_vars_check_pkcs7_2(&tmp, va, data);
                    if (status != EFI_SUCCESS) {
                        return status;
                    }
                } else if (secure_boot_is_active(uv)) {
                    return EFI_SECURITY_VIOLATION;
                }
            } else {
                status = uefi_vars_check_pkcs7_2(siglist, va, data);
                if (status != EFI_SUCCESS) {
                    return status;
                }
            }
